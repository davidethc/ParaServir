...
-- Trigger: limitar a 3 servicios por trabajador
CREATE OR REPLACE FUNCTION enforce_max_three_services()
RETURNS trigger AS $$
BEGIN
    IF (SELECT COUNT(*) FROM worker_services WHERE worker_id = NEW.worker_id) >= 3 THEN
        RAISE EXCEPTION 'Un trabajador no puede tener más de 3 servicios';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trg_max_three_services ON worker_services;
CREATE TRIGGER trg_max_three_services
BEFORE INSERT ON worker_services
FOR EACH ROW
EXECUTE FUNCTION enforce_max_three_services();
